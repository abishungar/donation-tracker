...
export default function EmailSettingsModal({ onClose }) {
  const [form, setForm] = useState({
    ...
    google_form_id: "", google_form_email_entry: "", google_form_name_entry: "", google_form_from_entry: "", google_form_subject_entry: "", google_form_body_entry: "", email_system_name: "Donation Tracker", email_from_address: ""
  });
  ...
  return <Modal title="Email Delivery Settings" onClose={onClose}>{loading?<p className="text-sm text-gray-400">Loading settings...</p>:<>
    ...
    {isForm ? <>
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-800 mb-4">The system submits these values to the Google Form for every email. Your Google Apps Script should use the submitted body as <b>htmlBody</b> if you want the email design to render.</div>
      <Field label="Google Form ID or URL"><input value={form.google_form_id} className={inputCls} onChange={e=>set("google_form_id",e.target.value)} placeholder="1FAIpQL... or full Google Form URL"/></Field>
      <div className="grid sm:grid-cols-2 gap-3"><Field label="System name / name to show"><input value={form.email_system_name} className={inputCls} onChange={e=>set("email_system_name",e.target.value)} placeholder="Donation Tracker"/></Field><Field label="From email address"><input type="email" value={form.email_from_address} className={inputCls} onChange={e=>set("email_from_address",e.target.value)} placeholder="sender@example.com"/></Field></div><Field label="Recipient email entry number"><input value={form.google_form_email_entry} className={inputCls} onChange={e=>set("google_form_email_entry",e.target.value)} placeholder="123456789"/></Field>
      <div className="grid sm:grid-cols-2 gap-3"><Field label="System name entry number"><input value={form.google_form_name_entry} className={inputCls} onChange={e=>set("google_form_name_entry",e.target.value)} placeholder="e.g. 123456789"/></Field><Field label="From address entry number"><input value={form.google_form_from_entry} className={inputCls} onChange={e=>set("google_form_from_entry",e.target.value)} placeholder="Optional"/></Field></div>
