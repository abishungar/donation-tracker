...
export default function EmailSettingsModal({ onClose }) {
  ...
  return <Modal title="Email Delivery Settings" onClose={onClose}>{loading?<p className="text-sm text-gray-400">Loading settings...</p>:<>
    ...
    {isForm ? <>
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-800 mb-4">The app submits these values to the Google Form. If your Form URL contains /d/e/, keep the full URL or published ID so the app can use the correct response endpoint. Your Google Form/Apps Script automation must be configured to actually send the email. Use the numeric entry IDs from the form fields (for example, 123456789). The default name/from values are submitted on every email unless a specific email send supplies an override.</div>
      <Field label="Google Form ID or URL"><input value={form.google_form_id} className={inputCls} onChange={e=>set("google_form_id",e.target.value)} placeholder="Paste the full /forms/d/... URL or ID"/></Field>
      ...
      <div className="grid sm:grid-cols-2 gap-3"><Field label="Default name to submit"><input value={form.google_form_default_name} className={inputCls} onChange={e=>set("google_form_default_name",e.target.value)} placeholder="Name shown by the email automation"/></Field><Field label="Default from email to submit"><input type="email" value={form.google_form_default_from} className={inputCls} onChange={e=>set("google_form_default_from",e.target.value)} placeholder="from@example.com"/></Field></div>
