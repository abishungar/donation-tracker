...
export default function Layout({ children }) {
  ...
  const SidebarContent=(
    <div className="flex flex-col h-full min-h-0">
      <div className="px-5 py-5 flex items-center gap-2 border-b border-white/10 shrink-0"><div className="w-8 h-8 rounded-lg bg-brand-500 flex items-center justify-center"><HeartHandshake size={18} className="text-white"/></div><span className="font-semibold text-white">Donation Tracker</span></div>
  ...
  return <div className="min-h-screen bg-gray-50">
    <aside className="hidden sm:flex fixed inset-y-0 left-0 z-30 w-64 bg-gray-900">{SidebarContent}</aside>
    <div className="sm:ml-64 min-h-screen">
      <div className="sm:hidden sticky top-0 z-20 bg-gray-900 flex items-center justify-between px-4 py-3"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-lg bg-brand-500 flex items-center justify-center"><HeartHandshake size={15} className="text-white"/></div><span className="font-semibold text-white text-sm">Donation Tracker</span></div><button onClick={()=>setOpen(true)} className="text-white p-1"><Menu size={22}/></button></div>
      {open&&<div className="sm:hidden fixed inset-0 z-40 flex"><div className="w-72 bg-gray-900 h-full"> <div className="flex justify-end px-3 pt-3"><button onClick={()=>setOpen(false)} className="text-white p-1"><X size={20}/></button></div>{SidebarContent}</div><div className="flex-1 bg-black/40" onClick={()=>setOpen(false)}/></div>}
