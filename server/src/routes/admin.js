...
router.get("/settings", main, async (req, res) => {
  ...
  const o = Object.fromEntries(
    rows.map((x) => [x.key, x.key === "smtp_app_password" ? "" : x.value])
  );
  res.json(o);
});

router.put("/settings", main, async (req, res) => {
  const allowed = ["smtp_user", "smtp_app_password", "smtp_from"];
  ...
  await writeLog(req, "UPDATE_SMTP_SETTINGS", {});
  res.json({ success: true });
});

// Save settings and immediately verify Gmail. No email is sent here.
router.post("/email/test", main, async (req, res) => {
  ...
  try {
    ...
    await sendMail(
      to,
      "Donation Tracker — Test Email",
      `<p>This is a test email from Donation Tracker.</p>
       <p>SMTP is working correctly for <strong>${result.user}</strong>.</p>`
    );
    await writeLog(req, "TEST_EMAIL_SENT", { to });
  ...
  for (const row of rows) {
    ...
    await prisma.contact.create({
      data: {
        ...
        email: get("email") ? String(get("email")).toLowerCase() : null,
