...
const { sendMail, passwordMail } = require("../utils/mailer");
...
router.post("/request-password-link", async (req,res)=>{
  ...
  if (user) {
    ...
    try { await sendMail(user.email,user.passwordSet?"Reset your Donation Tracker password":"Set your Donation Tracker password",passwordMail(user.name,link,user.passwordSet)); } catch(e) { return res.status(503).json({error:"Email is not configured. Ask the Main Admin to configure Gmail SMTP."}); }
  }
